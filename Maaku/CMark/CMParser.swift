...
public class CMParser {
    ...
    private func handleNode(_ node: CMNode, eventType: CMEventType) throws {
        ...
        case .list:
            ...
            case .ordered:
                if eventType == .enter {
                    delegate?.parser(parser: self, didStartOrderedListWithStartingNumber: node.listStartingNumber, tight: node.listTight)
                }
                else {
                    delegate?.parser(parser: self, didEndOrderedListWithStartingNumber: node.listStartingNumber, tight: node.listTight)
                }
            case .unordered:
                if eventType == .enter {
